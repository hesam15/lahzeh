import { useState, useEffect } from 'react';

const AudioVolumeSlider: React.FC = () => {
    const [volume1, setVolume1] = useState<number>(0.1);
    const [volume2, setVolume2] = useState<number>(0.1);
    const [volume3, setVolume3] = useState<number>(0.1);
    const [audioLoaded1, setAudioLoaded1] = useState(false);
    const [audioLoaded2, setAudioLoaded2] = useState(false);
    const [audioLoaded3, setAudioLoaded3] = useState(false);

    useEffect(() => {
        if (audioLoaded1) {
            const audio = document?.getElementById('audio1') as HTMLAudioElement;
            if (audio) {
                audio.volume = volume1;
            }
        }
    }, [volume1, audioLoaded1]);

    useEffect(() => {
        if (audioLoaded2) {
            const audio = document?.getElementById('audio2') as HTMLAudioElement;
            if (audio) {
                audio.volume = volume2;
            }
        }
    }, [volume2, audioLoaded2]);

    useEffect(() => {
        if (audioLoaded3) {
            const audio = document?.getElementById('audio3') as HTMLAudioElement;
            if (audio) {
                audio.volume = volume3;
            }
        }
    }, [volume3, audioLoaded3]);

    const handleAudioLoaded1 = () => setAudioLoaded1(true);
    const handleAudioLoaded2 = () => setAudioLoaded2(true);
    const handleAudioLoaded3 = () => setAudioLoaded3(true);

    return (
        <div>
            <audio id="audio1" src="/sounds/rain.mp3" autoPlay loop onCanPlay={handleAudioLoaded1} />
            <input
                type="range"
                min="0"
                max="1"
                step="0.1"
                value={volume1}
                onChange={(e) => setVolume1(Number(e.target.value))}
            />

            <audio id="audio2" src="/sounds/forest.mp3" autoPlay loop onCanPlay={handleAudioLoaded2} />
            <input
                type="range"
                min="0"
                max="1"
                step="0.1"
                value={volume2}
                onChange={(e) => setVolume2(Number(e.target.value))}
            />

            <audio id="audio3" src="/sounds/sea.mp3" autoPlay loop onCanPlay={handleAudioLoaded3} />
            <input
                type="range"
                min="0"
                max="1"
                step="0.1"
                value={volume3}
                onChange={(e) => setVolume3(Number(e.target.value))}
            />
        </div>
    );
};

export default AudioVolumeSlider;
