...
const AudioPlayerCircle = () => {
    ...
    const [volume, setVolume] = useState(0.5); // ولوم واقعی
    const sliderRef = useRef<HTMLInputElement>(null);
  
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const inputVal = parseFloat(e.target.value);
      const actualVolume = 1 - inputVal; // معکوس‌سازی ولوم
  ...
  useEffect(() => {
    ...
    if (slider) {
      const percent = (1 - volume) * 100;
      slider.style.background = `linear-gradient(to right, white ${percent}%, #444 ${percent}%)`;
    }
  }, [volume]);
