...
const seekAudio = (clientX: number, clientY: number) => {
    ...
    setProgress(percent);  // فقط پروگرس بار رو تغییر می‌دهیم
    const newTime = (percent / 100) * duration;  // محاسبه زمان جدید
    audioRef.current.currentTime = newTime;  // تغییر زمان صدا
};

// هنگام درگ یا کلیک روی پراگرس بار فقط وضعیت پخش تغییر نکند
const handleMouseDown = (e: React.MouseEvent) => {
    setIsDragging(true);
    seekAudio(e.clientX, e.clientY);  // زمانی که موس پایین میاد، فقط زمان رو تغییر بده
};

const handleMouseMove = (e: MouseEvent) => {
    if (isDragging) {
        seekAudio(e.clientX, e.clientY);  // اگر درگ میشه، زمان رو به‌روز کن
    }
};

const handleMouseUp = () => {
    setIsDragging(false);  // زمانی که موس بالا میره، درگینگ رو قطع کن
};

// تابع تغییر وضعیت پخش
const togglePlay = () => {
...
// برای جلوگیری از تداخل با پروگرس بار و وضعیت پخش
const handleClick = (e: React.MouseEvent) => {
    if (!isDragging) {
        togglePlay();  // فقط زمانی که در حال درگ نیستیم وضعیت پخش رو تغییر بدیم
    }
};
