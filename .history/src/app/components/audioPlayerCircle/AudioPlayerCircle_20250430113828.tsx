...
const AudioPlayerCircle = () => {
  ...
  const seekAudio = (clientX: number, clientY: number) => {
    ...
    // زاویه بین موس و مرکز دایره
    let theta = Math.atan2(dy, dx);
    theta -= Math.PI / 2;  // برای تنظیم شروع از نقطه بالای دایره
    if (theta < 0) theta += 2 * Math.PI;

    const percent = (theta / (2 * Math.PI)) * 100; // تبدیل به درصد
    setProgress(percent);  // بروزرسانی پروگرس بار
    const newTime = (percent / 100) * duration; // تبدیل درصد به زمان جدید
    audioRef.current.currentTime = newTime;  // تغییر زمان
};

const handleMouseDown = (e: React.MouseEvent) => {
    setIsDragging(true);  // فعال کردن حالت دراگ
    seekAudio(e.clientX, e.clientY);  // انتقال به موقعیت موس
};

const handleMouseMove = (e: MouseEvent) => {
    if (isDragging) {
        seekAudio(e.clientX, e.clientY);  // بروزرسانی موقعیت در حین کشیدن موس
    }
};

const handleMouseUp = () => {
    setIsDragging(false);  // وقتی موس رو رها کردیم، دراگینگ قطع میشه
};
