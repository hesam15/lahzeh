...
const AudioPlayerCircle const AudioPlayer: React.FC<AudioPlayerProps> = ({ data }) => {

    const audioRef = useRef<HTMLAudioElement>(null);
    const svgRef = useRef<SVGSVGElement>(null);

    const [isPlaying, setIsPlaying] = useState(false); // ← مقدار اولیه: false
    ...
    const handleVolumeChange = (event) => {
        ...
        setVolume(newVolume);
        if (audioRef.current) {
            audioRef.current.volume = newVolume; // تنظیم حجم صدا
        }
    };
        ...
        <div className="w-full flex flex-col items-center justify-center p-4">
            ...
            <div className="w-full flex items-center justify-center mt-4">
                ...
                <input
                    ...
                    min="0"
                    max="1"
                    step="0.01" // برای تنظیم دقیق‌تر حجم صدا
                    value={volume}
            ...
            <audio ref={audioRef} src={data?.narration?.file_path} preload="metadata" />

            <style jsx>{`
#myRange {
  transform: scaleX(-1); /* معکوس کردن نوار */
  width: 100%;
  height: 4px; /* کاهش ارتفاع */
  -webkit-appearance: none; /* حذف استایل پیش‌فرض */
  background: transparent; /* حذف پس‌زمینه اولیه */
}

#myRange::-webkit-slider-runnable-track {
  height: 4px;
  background: #ccc; /* رنگ پس‌زمینه (قسمت پر نشده) */
  border-radius: 2px;
}
...
#myRange::-webkit-slider-thumb {
  ...
  border-radius: 50%;
  background: #fff;
  margin-top: -4px; /* تراز کردن با نوار */
  cursor: pointer;
