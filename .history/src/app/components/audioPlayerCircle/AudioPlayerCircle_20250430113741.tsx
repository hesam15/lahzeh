...
const AudioPlayerCircle = () => {
  ...
  const seekAudio = (clientX: number, clientY: number) => {
    ...
    audioRef.current.currentTime = newTime;  // انتقال صدا به موقعیت جدید
};

const handleMouseDown = (e: React.MouseEvent) => {
    setIsDragging(true);
    seekAudio(e.clientX, e.clientY); // در ابتدا صدا رو به موقعیت جدید انتقال می‌دهیم
};

const handleMouseMove = (e: MouseEvent) => {
    if (isDragging) {
        seekAudio(e.clientX, e.clientY); // در حین کشیدن، وضعیت پروگرس بار و صدا تغییر می‌کنه
    }
};

const handleMouseUp = () => {
    setIsDragging(false); // وقتی موس رو رها کردیم
};
