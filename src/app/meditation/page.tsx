...
export default function Meditation() {
  ...
  useEffect(() => {
    ...
    const fetchSubCategories = async () => {
      try {
        const res = await axios.get(API_ENDPOINTS.meditationSubcategories(), apiHelpers.getRequestConfig(token));
  ...
  const category6 = categories.find((cat) => cat.id === '6');
